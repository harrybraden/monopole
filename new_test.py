...
def is_awc_multiple_root(k, x1, x2, x3):   # This will test if there are multiple roots; the analytic derivation assumes they are distinct
    ...
    k1 = sqrt(1-k**2)
    tol1 = 0.0002     # Had been using 0.007
    tol2 = 0.026     # Had been using 0.026
...
directory = "/Users/hwb/Desktop/numerical monopoles/testing_energydensity/x_axis/"

filename = (directory + 'k=%s' % k_value)

def write_point_to_file(points, filename):

    write_floats(filename, points)
...
def energy_density_on_x_axis(k_value, x0, x1, partition_size):
    ...
    for k in range(0, partition_size):
        ...
        bucket_value = value
        if(bucket_value > 4.0 or bucket_value < 0.0):
...
# p = energy_density_on_x_axis(k_value, 0.05, 5.05, 500)   # k, x-initial x-final, y-initial, y-final, partition size=no points between initial final
#
...
for k in range(0, 500):
    ...
    if is_awc_multiple_root(k_value, x1, 0.00, 0.00):
        print k,  -1
    else:
        print k, energy_density(k_value, x1, 0.00, 0.00)
